...
const isHorizontalScrollReachedEnd = (e) => e.target.scrollWidth - e.target.clientWidth - e.target.scrollLeft === 0;
...
function SessionPreviewList({ previewData, backgroundGradient }) {
	...
	return (
		<div className="w-full h-52 flex items-center">
			...
			<div
				className={cx('flex justify-between items-center gap-x-3  h-full w-full overflow-x-hidden')}
				onScroll={updateScrollLeft}
				ref={scrollBarRef}
			>
				{previewData.map((data) => (
					<SessionPreview key={data.title} data={data} backgroundGradient={backgroundGradient} />
				))}
			</div>
			<span className={cx({ visible: !isEndReached }, { invisible: isEndReached })} onClick={scrollToRight}>
				<RightArrow />
			</span>
		</div>
	);
